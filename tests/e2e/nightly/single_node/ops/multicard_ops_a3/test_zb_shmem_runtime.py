...
Verifies that on a real A3 box with Ascend SHMEM available:
  - ``ShmemMoERuntime.init`` brings up ``aclshmemx_init_attr`` on every rank.
...
def _worker(rank: int, world_size: int, results) -> None:
    try:
        ...
        assert isinstance(bundle, LowLatencyShmemTensors)
        ...
        alias = runtime.alias_tensor(
            qbundle.combine_x, [1024, 1024], torch.int8)
    ...
    for op_name in ("zb_shmem_init", "zb_shmem_alloc", "zb_shmem_alloc_tensor",
        ...
        if not hasattr(torch.ops._C_ascend, op_name):
            raise AssertionError(
                f"torch.ops._C_ascend.{op_name} missing; rebuild vllm_ascend_C "
                "against Ascend SHMEM")
