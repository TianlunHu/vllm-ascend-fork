...
#include "../op_kernel/shmem_moe_distribute_dispatch_zero_buffer_tiling.h"
...
namespace optiling {
static void PrintTilingDataInfo(const char *nodeName, ShmemMoeDistributeDispatchZeroBufferTilingData &tilingData)
{
...
static ge::graphStatus CheckAndSetGroupInfo(const gert::TilingContext *context, const char *nodeName,
                                            ShmemMoeDistributeDispatchZeroBufferTilingData &tilingData)
{
...
static ge::graphStatus CheckAndSetSpecialExpertInfo(const gert::TilingContext *context, const char *nodeName,
                                                    ShmemMoeDistributeDispatchZeroBufferTilingData &tilingData, bool &isSetCommAlg)
{
    auto attrs = context->GetAttrs();
    ...
    OP_LOGD(nodeName, "ShmemMoeDistributeDispatchZeroBuffer isSetCommAlg = %d\n", isSetCommAlg);
    tilingData.moeDistributeDispatchZeroBufferInfo.zeroComputeExpertNum = static_cast<int32_t>(zeroComputeExpertNum);
    OP_LOGD(nodeName, "ShmemMoeDistributeDispatchZeroBuffer zeroComputeExpertNum = %d\n",
                                               ...
                                               ShmemMoeDistributeDispatchZeroBufferTilingData &tilingData, bool &isSetCommAlg)
{
    ...
    OP_LOGD(nodeName, "lastDim = %d, ShmemMoeDistributeDispatchZeroBuffer cumSumUBMinValue = %d\n", lastDim,
...
static bool CheckSharedAttrs(const char *nodeName, const ShmemMoeDistributeDispatchZeroBufferTilingData &tilingData)
{
...
static bool CheckCommAlgAttrs(const char *nodeName, const ShmemMoeDistributeDispatchZeroBufferTilingData &tilingData,
...
static ge::graphStatus CheckAttrs(const gert::TilingContext *context, const char *nodeName,
                                  ShmemMoeDistributeDispatchZeroBufferTilingData &tilingData, uint32_t &localMoeExpertNum,
                                  bool isActiveMask, bool isSetCommAlg)
{
    ...
    OP_LOGD(nodeName, "ShmemMoeDistributeDispatchZeroBuffer *globalBsPtr = %ld, bs = %ld, epWorldSize = %u\n", *globalBsPtr, xDim0,
            epWorldSize);
    OP_TILING_CHECK(
...
static ge::graphStatus CheckTensorShape(const gert::TilingContext *context, const char *nodeName,
                                        ShmemMoeDistributeDispatchZeroBufferTilingData &tilingData, const uint32_t quantMode,
...
static void SetHcommCfg(const gert::TilingContext *context, ShmemMoeDistributeDispatchZeroBufferTilingData *tiling)
{
    auto attrs = context->GetAttrs();
    // auto groupEpPtr = attrs->GetAttrPointer<char>(static_cast<int>(ATTR_GROUP_EP_INDEX));
    // auto groupTpPtr = attrs->GetAttrPointer<char>(static_cast<int>(ATTR_GROUP_TP_INDEX));
    ...
    OP_LOGD(nodeName, "ShmemMoeDistributeDispatchZeroBuffer groupEp = %s, groupTp = %s", groupEp.c_str(), groupTp.c_str());
...
static ge::graphStatus CheckWinSize(ShmemMoeDistributeDispatchZeroBufferTilingData &tilingData, const char *nodeName,
...
static ge::graphStatus SetAivInfo(gert::TilingContext *context, ShmemMoeDistributeDispatchZeroBufferTilingData *tilingData,
                                  const char *nodeName)
{
...
static ge::graphStatus GetParamsAndSetTilingData(const gert::TilingContext *context,
                                                 ShmemMoeDistributeDispatchZeroBufferTilingData *tilingData, bool &isActiveMask,
...
{
    const char *nodeName = context->GetNodeName();
    ShmemMoeDistributeDispatchZeroBufferTilingData *tilingData = context->GetTilingData<ShmemMoeDistributeDispatchZeroBufferTilingData>();
    ...
    OP_LOGI(nodeName, "Enter ShmemMoeDistributeDispatchZeroBuffer tiling check func.");
...
IMPL_OP_OPTILING(ShmemMoeDistributeDispatchZeroBuffer)
