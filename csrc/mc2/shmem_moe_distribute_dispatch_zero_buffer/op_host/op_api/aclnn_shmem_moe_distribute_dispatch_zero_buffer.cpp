#include "aclnn_shmem_moe_distribute_dispatch_zero_buffer.h"
...
extern aclnnStatus aclnnInnerShmemMoeDistributeDispatchZeroBufferGetWorkspaceSize(
...
extern aclnnStatus aclnnInnerShmemMoeDistributeDispatchZeroBuffer(void *workspace, uint64_t workspaceSize,
                                                                   aclOpExecutor *executor, aclrtStream stream);

extern "C" void __attribute__((weak)) NnopbaseSetHcclServerType(void *executor, NnopbaseHcclServerType sType);

aclnnStatus aclnnShmemMoeDistributeDispatchZeroBufferGetWorkspaceSize(
...
{
    return aclnnInnerShmemMoeDistributeDispatchZeroBufferGetWorkspaceSize(
...
aclnnStatus aclnnShmemMoeDistributeDispatchZeroBuffer(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
                                                      aclrtStream stream)
{
    if (NnopbaseSetHcclServerType) {
        NnopbaseSetHcclServerType(executor, NNOPBASE_HCCL_SERVER_TYPE_MTE);
    }
    return aclnnInnerShmemMoeDistributeDispatchZeroBuffer(workspace, workspaceSize, executor, stream);
}
