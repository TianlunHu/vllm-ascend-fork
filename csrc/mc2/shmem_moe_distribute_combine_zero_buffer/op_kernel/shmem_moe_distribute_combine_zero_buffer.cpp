#include "shmem_moe_distribute_combine_zero_buffer.h"
...
__aicore__ inline void ExecShmemMoeDistributeCombineZeroBuffer(GM_ADDR expandX, GM_ADDR expertIds, GM_ADDR assistInfoForCombine,
...
{
    GET_TILING_DATA_WITH_STRUCT(ShmemMoeDistributeCombineZeroBufferTilingData, tilingData, tilingGM);
    ShmemMoeDistributeCombineZeroBuffer<TemplateMC2TypeFunc> op;
...
extern "C" __global__ __aicore__ void shmem_moe_distribute_combine_zero_buffer(
...

{
    ...
    REGISTER_TILING_DEFAULT(ShmemMoeDistributeCombineZeroBufferTilingData);
    TPipe pipe;

#if (ORIG_DTYPE_EXPAND_X == DT_BF16 || ORIG_DTYPE_EXPAND_X == DT_FLOAT16)
    if (TILING_KEY_IS(10100)) {  // tp=2 IsInt8Quant=0
        ExecShmemMoeDistributeCombineZeroBuffer<DTYPE_EXPAND_X, DTYPE_X, int32_t, true, false>(
    ...
    if (TILING_KEY_IS(10000)) {  // tp=1 IsInt8Quant=0
        ExecShmemMoeDistributeCombineZeroBuffer<DTYPE_EXPAND_X, DTYPE_X, int32_t, false, false>(
            expandX, expertIds, assistInfoForCombine, epSendCount, tpSendCount, scales, xActiveMask, sharedExpertX,
            elasticInfo, oriX, constExpertAlpha1, constExpertAlpha2, constExpertV, XOut, workspaceGM, tilingGM, &pipe);
    }
    if (TILING_KEY_IS(10120)) {  // tp=2 IsInt8Quant=1
        ExecShmemMoeDistributeCombineZeroBuffer<DTYPE_EXPAND_X, DTYPE_X, int32_t, true, true>(
            expandX, expertIds, assistInfoForCombine, epSendCount, tpSendCount, scales, xActiveMask, sharedExpertX,
            elasticInfo, oriX, constExpertAlpha1, constExpertAlpha2, constExpertV, XOut, workspaceGM, tilingGM, &pipe);
    }
    if (TILING_KEY_IS(10020)) {  // tp=1 IsInt8Quant=1
        ExecShmemMoeDistributeCombineZeroBuffer<DTYPE_EXPAND_X, DTYPE_X, int32_t, false, true>(
            expandX, expertIds, assistInfoForCombine, epSendCount, tpSendCount, scales, xActiveMask, sharedExpertX,
            elasticInfo, oriX, constExpertAlpha1, constExpertAlpha2, constExpertV, XOut, workspaceGM, tilingGM, &pipe);
    }
#endif
