#include "aclnn_shmem_moe_distribute_combine_zero_buffer.h"
...
extern aclnnStatus aclnnInnerShmemMoeDistributeCombineZeroBufferGetWorkspaceSize(
...
extern aclnnStatus aclnnInnerShmemMoeDistributeCombineZeroBuffer(void *workspace, uint64_t workspaceSize,
                                                                  aclOpExecutor *executor, aclrtStream stream);

extern "C" void __attribute__((weak)) NnopbaseSetHcclServerType(void *executor, NnopbaseHcclServerType sType);

aclnnStatus aclnnShmemMoeDistributeCombineZeroBufferGetWorkspaceSize(
    const aclTensor *expandX, const aclTensor *expertIds, const aclTensor *assistInfoForCombine, const aclTensor *epSendCount,
    const aclTensor *scales, const aclTensor *tpSendCount, const aclTensor *xActiveMask, const aclTensor *activationScale,
    const aclTensor *weightScale, const aclTensor *groupList, const aclTensor *expandScales, const aclTensor *sharedExpertX,
    const aclTensor *elasticInfo, const aclTensor *oriX, const aclTensor *constExpertAlpha1, const aclTensor *constExpertAlpha2,
    ...
    int64_t copyExpertNum, int64_t constExpertNum, const aclTensor *XOut, uint64_t *workspaceSize, aclOpExecutor **executor)
{
    return aclnnInnerShmemMoeDistributeCombineZeroBufferGetWorkspaceSize(
...
aclnnStatus aclnnShmemMoeDistributeCombineZeroBuffer(void *workspace, uint64_t workspaceSize, aclOpExecutor *executor,
                                                     aclrtStream stream)
{
    if (NnopbaseSetHcclServerType) {
        NnopbaseSetHcclServerType(executor, NNOPBASE_HCCL_SERVER_TYPE_MTE);
    }
    return aclnnInnerShmemMoeDistributeCombineZeroBuffer(workspace, workspaceSize, executor, stream);
}
